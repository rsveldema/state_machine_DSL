...
int main(int argc, char **argv)
{
  ...
  if (random_path)
    {
      RandomPathChecker<{{state_machine_name}}> rc(p, max_step_count_per_machine);
      rc.run();
    }
  else
    {
      Modelchecker<{{state_machine_name}}> mc(p, use_statistical_analysis, max_step_count_per_machine);
      mc.run();
    }
  return 0;
}
