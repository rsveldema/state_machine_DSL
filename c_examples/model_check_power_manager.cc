...
int main(int argc, char **argv)
{
  ...
  if (random_path)
    {
      RandomPathChecker<PowerManager> rc(p, max_step_count_per_machine);
      rc.run();
    }
  else
    {
      Modelchecker<PowerManager> mc(p, use_statistical_analysis, max_step_count_per_machine);
      mc.run();
    }
  return 0;
}
